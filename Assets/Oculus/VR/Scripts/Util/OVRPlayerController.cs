...
public class OVRPlayerController : MonoBehaviour
{
	...
	private float crouchVar = 0.0f;
	private float crouchSpeed = 5.0f;
	private float nextCrouch = 0.0f;

	private bool isRolling = false;
	private float rollVar = 0.0f;
	...
	/// <summary>
	/// The force applied to the character when jumping. Originally 0.3f
	/// </summary>
	...
	{		
		if (OVRInput.GetDown(OVRInput.Button.Four) && Time.time > nextFire)
		{
			Quaternion localRot = OVRInput.GetLocalControllerRotation(OVRInput.Controller.LTouch);

			Transform leftHandPosition = transform.Find("OVRCameraRig").Find("TrackingSpace").Find("LeftHandAnchor").Find("LHandTransform");

			Quaternion handrot = transform.rotation * localRot;

			GameObject spawnedProjectile = Instantiate(projectile, leftHandPosition.position, handrot);
			spawnedProjectile.GetComponent<Projectile>().direction = leftHandPosition.forward;
	...
	public void HandleDodgeRoll()
	{
		...
		{
			Quaternion ort = transform.rotation;
			MoveThrottle += ort * (1f * transform.lossyScale.x * 1 *
									   BackAndSideDampen * Vector3.left);
		}

		// if (OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger) == 1 && Time.time > nextRoll)
		// {
		// 	Debug.Log("Rolling");
		// 	isRolling = true;
		// 	rollVar = -5f;
		// 	nextRoll = Time.time + 5f;
		// }
	}
	public void Roll()
	{
		// if (OVRInput.GetDown(OVRInput.Button.Two)) {
		// 	transform.RotateAround(CameraRig.centerEyeAnchor.position, Vector3.forward, 20);
		// }

		rollVar += rollSpeed * Time.deltaTime;
		float height = .04f * rollVar * rollVar + 1;
	...
	public void HandleCrouch()
	{
		if (!isCrouching)
		{
			if (!isCrouched)
			{
				if (OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger) > .5 && Time.time > nextCrouch)
				{
					isCrouching = true;
					crouchVar = 2f;
				} 
	...
	public void Crouch()
	{
		crouchVar -= crouchSpeed * Time.deltaTime;
		if (!isCrouched)
		{

			Controller.height = crouchVar;
			...
			Controller.height = 3f - crouchVar;
		...
		// if (OVRInput.GetDown(OVRInput.Button.Three) && Time.time > nextCrouch)
		// {
		// 	isCrouching = !isCrouching;
		// 	nextCrouch = Time.time + 0.1f;
		// }
	}
	...
	{
		if (Controller.isGrounded)
			MoveThrottle += new Vector3(0, 10, 0);
